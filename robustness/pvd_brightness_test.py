...
from robustness.common import ASSETS_DIR, RESULTS_DIR, ensure_test_image, random_ascii_message
...
def run(image_path: Path | None = None) -> None:
    ...
    message = random_ascii_message(msg_bytes, seed=2)
    ...
    for factor in [0.6, 0.75, 0.9, 1.1, 1.25, 1.5]:
        attacked = ImageEnhance.Brightness(stego.convert("RGB")).enhance(float(factor))
        out_path = RESULTS_DIR / f"pvd_brightness_{factor:.2f}.png"
        ...
        print(
            f"[PVD][BRIGHT x{factor:.2f}] ok={ok} "
